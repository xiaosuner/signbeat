...
const MusicGame = () => {
        ...
        const initializeGestureRecognizer = async () => {
            ...
            navigator.mediaDevices.getUserMedia(constraints).then((stream) => {
                if (videoRef.current) {
                    ...
                    videoRef.current.addEventListener("loadeddata", () => predictWebcam(gestureRecognizer));
                }
            });
        };
        ...
        const predictWebcam = async (gestureRecognizer) => {
            ...
            const detectGesture = async () => {
                ...
                if (results.gestures.length > 0) {
                    ...
                    if (!isReady && gesture.categoryName === "Thumb_Up" && gesture.score * 100 > 70) {
                        setStartText("Start");
                        setTimeout(() => {
                            setIsReady(true);
                            setIsPlaying(true);
    ...
    return (
        <Layout style={{ transition: "all 0.5s ease" }}>
            <Content style={{ margin: "0 20rem", position: "relative", textAlign: "center" }}>
                ...
                ></canvas>
                {!isReady && (
                    <div style={{ zIndex: 11, position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", fontSize: "2rem", background: "rgba(0,0,0,0.5)", color: "white", padding: "1rem", borderRadius: "1rem" }}>
                        {startText}
                    </div>
                )}
                <div style={{ zIndex: 11 }}>
